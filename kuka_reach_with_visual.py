...
class KukaReachVisualEnv(gym.Env):
    ...
    def reset(self):
        ...
        table_uid = p.loadURDF("table/table.urdf", basePosition=[0.5, 0, -0.63])

        p.changeVisualShape(table_uid, -1, rgbaColor=[1, 1, 1, 1])

        self.slot = p.loadURDF("FYPLONG/slotmodel1/URDF/slotmodel1.urdf", useFixedBase=True, basePosition=[0.72, 0, 0.014],
                               baseOrientation=orientation_wall2)
        self.object_id = p.loadURDF("FYPLONG/boltmodel/urdf/boltmodel.urdf", basePosition=[0.72, 0, 0],
